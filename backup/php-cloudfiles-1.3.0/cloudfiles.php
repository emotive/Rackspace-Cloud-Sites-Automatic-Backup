...
class CF_Connection
{
    ...
    function get_containers()
    {
        list($status, $reason, $container_info) =
                $this->cfs_http->list_containers_info();
        #if ($status == 401 && $this->_re_auth()) {
 ...
 *       object_count and bytes_used instance variables may either be a
 *       integer or float.
 *
 * @package php-cloudfiles
 */
class CF_Container
{
     ...
     * $next_ten = $images->get_objects(10,10);
     *
     * # Grab images starting with "birthday_party" and default limit/marker
     * # to match all photos with that prefix
     * #
    ...
    {
        if ($path_name[0] == '/') {
            $path_name = mb_substr($path_name, 1);
        }
...
class CF_Object
{
    ...
    function write($data=NULL, $bytes=0, $verify=True)
    {
        ...
        if (!is_resource($data)) {
            ...
            rewind($fp);
            $close_fh = True;
            $this->content_length = (float) mb_strlen($data);
        ...
        /* FIXME: test if this works with string AND resource (or string only) */
     ...
     * into a local variable (memory) before computing the checksum!  You
     * should ensure that you have enough RAM to support this.
     *
